...
class FastNavEnv(gym.Env):
    ...
    def reset(self, *, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        if self.scenario_factory is not None:
            # An explicit seed selects one exact arena, which is what the
            # evaluation harness needs for reproducibility. With no seed, draw a
            # fresh arena for this episode. Without this branch a training run
            # sees only as many arenas as there are parallel environments,
            # because SB3 seeds each one once and never again, and the policy
            # overfits to a handful of layouts while appearing to learn.
            if seed is not None:
                self.scenario = self.scenario_factory(seed)
            else:
                self.scenario = self.scenario_factory(
                    int(self.np_random.integers(0, 1_000_000)))
        self._reset_internal_state()
        self._spawn_dynamic()
        ...
        if self.randomise_start:
            for _ in range(50):
                w, h = self.scenario.bounds
                self.x = float(self.np_random.uniform(0.6, w - 0.6))
                self.y = float(self.np_random.uniform(0.6, h - 0.6))
                self.theta = float(self.np_random.uniform(-math.pi, math.pi))
                if self._clearance() > 0.25:
                    break
            else:
                self.x, self.y, self.theta = self.scenario.start_pose

        if self.domain_randomisation:
            self.lidar_noise_std = float(self.np_random.uniform(0.005, 0.04))

        self._prev_goal_distance = self._goal_relative()[0]
        obs = self._observe()
        return obs, {"scenario": self.scenario.name, "reward_fn": self.reward_name}
