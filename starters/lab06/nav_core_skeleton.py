...
Optional Lab 6 implementation extension. This is the file you edit. The geometry, the raycasting and the
...
This module implements FastNavEnv: a NumPy environment used for both training
and evaluation in Labs 8-10. RobotSpec, ObsSpec and build_observation define its
observation/action contract. A future ROS/Gazebo policy adapter must reproduce
that contract and validate timing, frames and reset behaviour. That complete
adapter is not implemented in this repository.
...
# ---------------------------------------------------------------------------
# Reward functions. A second-reward comparison is an optional Lab 8 extension.
...
class FastNavEnv(gym.Env):
    ...
    Labs 8-10 use this for training and matched surrogate evaluations. It has
    no physics engine, ROS integration or built-in rendering. Measure throughput
    on the teaching machine before choosing a training budget. Lab 9 varies
    observation noise and delay here; it does not demonstrate Gazebo transfer.
    """
...
def default_training_scenario(seed: int = 0) -> Scenario:
    """A modest room with pillars, used as the Lab 6 and Lab 8 default."""
