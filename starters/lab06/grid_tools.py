"""
Lab 6 starter: bridging a ROS OccupancyGrid to the planner you wrote yourself.

In the planning notebook you ran BFS, Dijkstra and A* on a NumPy array you made
up. Nav2 plans on a nav_msgs/OccupancyGrid produced by SLAM Toolbox. These
functions are the join between the two, and once you have them your own A* runs
on the same map the production planner is using.
"""
...
def to_binary_obstacle_map(grid: np.ndarray, occupied_threshold: int = 65,
    ...
    unmapped areas, which is what Nav2 does by default with allow_unknown set to
    true. Treating it as blocked is safer and more conservative. Try both in
    Exercise 6.1 and look at what changes.
    """
