"""
Tests for Lab 3. The occupancy grid and its inverse sensor model.

    python3 -m pytest starters/lab03 -q                              the reference
    ARC_OCCUPANCY=occupancy_skeleton python3 -m pytest starters/lab03 -q   your own
"""
...
def test_point_just_below_origin_does_not_alias_first_cell():
    m = fresh()
    assert m.world_to_cell(-0.01, 0.0) == (0, -1)
    assert not m.in_bounds(*m.world_to_cell(-0.01, 0.0))
    assert m.world_to_cell(0.0, -0.01) == (-1, 0)
